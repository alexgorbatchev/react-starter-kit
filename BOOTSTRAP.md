...
3. **Configure Your Project Environment**
   ...
   - **`DATABASE_URL`**: Connects to your local Postgres database. By default it is `postgres://postgres:postgres@localhost:5432/example`.
     _If you are running multiple projects, change `example` to a unique database name (e.g., `my_new_project_db`) to prevent data collisions._
   ...
   _(Note: OTP codes print directly to your terminal when `ENVIRONMENT=development`. However, if you explicitly trigger full email verifications or password resets without configuring `RESEND_API_KEY` and `RESEND_EMAIL_FROM`, the app will throw an error.)_
